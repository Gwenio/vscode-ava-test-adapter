...
const source = path.resolve(process.cwd(), 'deps', 'node_modules')

const dependencies = fs
	.readFile(path.join('deps', 'package.json'))
	.then(raw => {
...
exports.prepare = async function() {
	...
	return new Promise((resolve, reject) =>
		stream.pipeline(
			...
			sourcemap.write('.'),
			js.restore,
			dest(`tmp${path.sep}node_modules`),
			dest(`dist${path.sep}node_modules`),
			error => {
